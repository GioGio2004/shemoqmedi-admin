...
const NAV_ITEMS = [
  ...
  { label: "Storefront", href: "/dashboard/storefront",   icon: Store,           exact: false },
  ...
  return (
    ...
    >
      <div className={cn(
        ...
        isActive ? "bg-white/15 text-white" : "bg-white/5 text-zinc-500 group-hover:bg-white/10 group-hover:text-white"
      )}>
        <Icon className="h-3.5 w-3.5" />
      </div>
      {!collapsed && <span className="truncate">{item.label}</span>}
      {!collapsed && isActive && <div className="ml-auto h-1.5 w-1.5 shrink-0 rounded-full bg-white" />}
    </Link>
  );
}
...
function SidebarContent({ onNavClick }: { onNavClick?: () => void }) {
  ...
  const collapsed = useContext(SidebarCtx);
  const convexRole = useQuery(api.memberships.getMyRole, { orgId: organization?.id });
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Logo / brand */}
      <div className={cn("px-4 pt-1 pb-5", collapsed && "px-2 flex justify-center")}>
        {collapsed ? (
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[13px] font-medium leading-none text-white">Shemoqmedi</p>
      ...
      {!collapsed && (
        <div className="px-3 mb-4">
          <p className="mb-2 px-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-zinc-600">Workspace</p>
          <WorkspaceDisplay />
        </div>
      )}
      ...
      <nav className="flex-1 px-3">
        {!collapsed && <p className="mb-2 px-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-zinc-600">Navigation</p>}
      ...
      {!collapsed && (
        <div className="px-4 py-4 mt-auto border-t border-white/10">
          <p className="text-[10px] text-zinc-600 text-center font-mono">v1.0.0</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="flex h-[100dvh] flex-col overflow-hidden bg-black">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
          ...
          {/* ── Page content ─────────────────────────────────────────────────── */}
          <main className={cn("flex-1 min-h-0", isAiManager ? "overflow-hidden" : "overflow-y-auto bg-black")}>
            {isAiManager ? (
